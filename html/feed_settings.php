...
<body>
    ...
    <div class="modal-content">
        ...
        <table id="feedLogTable" style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr>
                    <th style="border: 3px solid #ccc; padding: 8px;">Time</th>
                    <th style="border: 3px solid #ccc; padding: 8px;">Amount</th>
                    <th style="border: 3px solid #ccc; padding: 8px;">Notes</th>
                </tr>
            </thead>
            <tbody>
...
function fetchFeedLogs() {
    $.ajax({
        url: './feed_settings_ajax/get_feed_logs.php', // Endpoint for logs
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            ...
            response.forEach(log => {
                ...
                row.innerHTML = `
                    <td style="border: 1px solid #ccc; padding: 8px;">${formatTimeTo12Hour(log.feed_time)}</td>
                    <td style="border: 1px solid #ccc; padding: 8px;">${log.amount}</td>
                    <td style="border: 1px solid #ccc; padding: 8px;">${log.motor_runtime}</td>
                `;
...
window.onclick = function(event) {
    ...
    var labels = ["10:00 AM", "11:00 AM", "12:00 PM"]; // Sample times
    var data = [5, 10, 15]; // Sample amounts
    ...
    $.ajax({
        url: './get_feed_history.php', // Update with your file path if needed
        type: 'POST',
        dataType: 'json',
        data: { selectedDate: selectedDate }, // Send selected date to the server
        success: function(response) {
            updateChart(response); // Call updateChart with the response data
        },
        error: function(error) {
            console.error('Error fetching feed history:', error);
        }
