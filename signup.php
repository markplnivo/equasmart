...
$nameErr = $contactErr = $emailErr = $passwordErr = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['contactNumber'])) {
        ...
        if (!preg_match("/^\d{11}$/", $contact)) {
            $contactErr = "Mobile number should be exactly 11 digits long.";
        }
    }
    ...
    if (empty($_POST["email"])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if (!in_array($domain, $allowedDomains)) {
                $emailErr = "Only Yahoo and Gmail email addresses are allowed";
            }
        }
    }
    ...
    if (empty($nameErr) && empty($contactErr) && empty($emailErr) && empty($passwordErr)) {
        // Check if the email is already used
        $emailCheck = $conn->prepare("SELECT * FROM customer WHERE customer_email = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO users (username, password, access, is_verified) VALUES (?, ?, ?, ?)");
        $access = 2;
        $is_verified = 0;
        $stmt->bind_param("ssii", $name, $hashed_password, $access, $is_verified);
        if ($stmt->execute()) {
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'jrafon0710@gmail.com';
                $mail->Password = 'tcdvqlvmjcbgqhlk';
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // Use 'tls' or 'ssl'
                $mail->Port = 465;

                // Recipients
                $mail->setFrom('jrafon0710@gmail.com', 'eQua Smart Company'); 
        ...
        <div class="right">
            ...
            <form action="signup.php" method="post">
                ...
                <div class="form-group">
                    ...
                    <label for="terms_and_conditions">I agree to the <a href="javascript:void(0);" id="showMoreLink">Terms of User (show more)</a></label>
                </div>
                <div class="form-action">
                    <button type="submit" name="registrationSubmit">Submit</button>
                </div>
            </form>
