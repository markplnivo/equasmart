        ...
        @media (max-width: 1010px) {
            ...
            *,
            ...
            .side-by-side-wrapper {
                display: flex;
                flex-direction: column;
                /* Stack date-picker and log-list-container vertically*/
                width: 100%;
                margin-top: 15%;
                margin-inline: 0;
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($interval) {
        ...
        if ($interval === 'daily') {
            $query = "SELECT AVG(Value) as average, $dateColumn FROM $table WHERE DATE($dateColumn) = ? GROUP BY $dateColumn";
        ...
        // Output results in a readable format for the log div
        foreach ($logs as $log) {
            echo "<p>Average: " . htmlspecialchars($log['average']) . "</p>";
            echo "<p>Date/Interval: " . htmlspecialchars($log[$dateColumn] ?? ($log['week'] ?? $log['month'] ?? $log['year'])) . "</p>";
            echo "<hr>";
        }
    }
}
...
<script>
    ...
    // Function to log date or interval without immediate display
    function logDate() {
