...
try {
    ...
    if (isTestRunning()) {
        echo "A test is already in progress. Skipping schedule check.";
        exit;
    }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (in_array($currentDay, $daysOfWeek) && in_array($currentTime, $scheduledTimes)) {
            // Set test as running
            setTestRunning(true);
